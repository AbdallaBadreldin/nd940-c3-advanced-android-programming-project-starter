...
class MainActivity : AppCompatActivity() {
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (id == downloadID) {
                ...
                val c: Cursor = manager.query(downloadManagerQuery)
                if (c.moveToFirst()) {
                    val status = c.getInt(c.getColumnIndex(DownloadManager.COLUMN_STATUS))
                    if (status == DownloadManager.STATUS_SUCCESSFUL) {
                        buildNotification()
                        editor.putString("download status", "Success")
                        editor.apply()
                    } else {
                        buildNotification()
                        editor.putString("download status", "Fail")
                        editor.apply()
                    }
                }
                c.close()
    ...
    private fun download(URL: String, file: String) {
        ...
        editor.putString("download name", file)
        editor.apply()
    }
